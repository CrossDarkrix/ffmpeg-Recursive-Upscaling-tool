...
def convert(fname):
	if fname.split('.')[-1].lower() == 'png':
		...
		try:
			img.resize((img.width * round(float(sys.argv[2])), img.height * round(float(sys.argv[2]))), Image.Resampling.LANCZOS).save('{} (x{}).png'.format(fname.split('.')[0], sys.argv[2]))
		except AttributeError:
			img.resize((img.width * round(float(sys.argv[2])), img.height * round(float(sys.argv[2]))), Image.LANCZOS).save('{} (x{}).png'.format(fname.split('.')[0], sys.argv[2]))
		try:
			os.remove(fname)
		except:
